...
public class Calc_data {
	...
	protected TOF_data[] calculated_tofs;

	protected String title;
	protected int num_poes, num_total_poes;
	protected float ion_m_e;

	protected int num_beam_ang_segs, num_det_ang_segs, num_ionizer_segs;
	protected boolean is_ionizer_gaussian;
	protected float[] ionizer_prob_dist;    // Normalized probability distribution
	protected float[] ionizer_distance;     // Array of distances from the ionizer center (in cm)
	protected float[] beam_velocities;    // In m/s
	protected float[] bm_vel_prob_dist, bm_vel_squared;   // Boltzmann distrib or input data

	protected float[] lab_velocities, lab_vel_squared;

	protected float[] cos_theta_lab, sin_theta_lab, cos_polar_angle, sin_polar_angle;
	protected float[] depolar_major, depolar_minor;

	protected float alpha, speed_ratio;
	protected int num_beam_vel_segs;
	protected float min_lab_velocity, max_lab_velocity;

	protected float flight_length, beam_angular_width, detector_angular_width, ionizer_length;
	protected float starting_time, ending_time;
	protected int num_tof_points, num_lab_vel_segs, num_tofs;
	protected float[] tof_lab_angles, tof_polarization_angles, depolarization;
	protected boolean[] tof_is_polarized;

	protected double[] cos_theta_beam, sin_theta_beam;
	protected double[] cos_theta_detector, sin_theta_detector;

	protected float[][] cos_phi_beam, sin_phi_beam;
	protected float[][] cos_phi_detector, sin_phi_detector;

	protected float[][][][][] TOF_2cos_gamma;  // YIKES!!!

	protected POE_calc_data[] data_for_poes, temp_data_for_poes;

	protected float[][][][] min_u_squared_array, max_u_squared_array;

	protected boolean bm_angle_has_changed, det_angle_has_changed, is_number_density_calc;

	protected int old_num_tofs, old_num_theta_bm, old_num_theta_det;
	protected int[] included_tof_num_array;

	protected JFrame main_frame_window;
	protected MessageDialog message_dialog;
	...
	public Calc_data(int title_calc_number, int num_tot_poes, float ionizer_len,
			JFrame main_window)
	{
		int i;
		...
		data_for_poes = new POE_calc_data[num_total_poes];

		// Initialize all POE_calc_data structs
		for(i = 0; i < num_total_poes; i++)
		{
			data_for_poes[i] = new POE_calc_data();
	...
	public void SetCalcNumber(int calc_number)  {CalcNumber = calc_number;}
	public void SetNumPOEs(int new_value)  {num_poes = new_value;}
	public void SetTotalNumPOEs(int new_value)  {num_total_poes = new_value;}
	public void AddPOE(){
		num_total_poes++;
		int i, tof_num;

		float[][][] temp_tof_min_array, temp_tof_max_array;
		float[][][] this_tof_min_u_squared_array, this_tof_max_u_squared_array;

		if(min_u_squared_array != null)
		{
			for(tof_num = 0; tof_num < num_tofs; tof_num++)
			{
				...
				temp_tof_min_array = new float[num_total_poes][][];
				temp_tof_max_array = new float[num_total_poes][][];
				for(i = 0; i < (num_total_poes - 1); i++)
				{
					temp_tof_min_array[i] = this_tof_min_u_squared_array[i];
					temp_tof_max_array[i] = this_tof_max_u_squared_array[i];
				}
				temp_tof_min_array[num_total_poes - 1] = null;
				temp_tof_max_array[num_total_poes - 1] = null;

				min_u_squared_array[tof_num] = temp_tof_min_array;
				max_u_squared_array[tof_num] = temp_tof_max_array;
			}
		}
	}

	public void RemovePOE(int poe_number){
		// Used to reset arrays which store information about minimum and maximum u^2
		// for a given P(E)
		int i, j, tof_num, this_num_channels;

		POE_calc_data current_poe_data;
		...
		if(min_u_squared_array != null)
		{
			for(tof_num = 0; tof_num < num_tofs; tof_num++)
			{
				...
				for(i = 0; i < num_total_poes; i++)
				{
					...
					{
						temp_tof_min_array[i] = this_tof_min_u_squared_array[i + 1];
						temp_tof_max_array[i] = this_tof_max_u_squared_array[i + 1];
					}
				}

				current_poe_data = data_for_poes[poe_number];
	...
	public void SetIon_m_e(float m_e_value)  {ion_m_e = m_e_value;}
	...
	public void SetFlightLen(float flight_len)  {flight_length = flight_len;}  // When flight length changes
	...
	public void SetIncludedTOFNumArray(int[] new_array)
	{
		included_tof_num_array = new_array;
	}

	public void SetPOECalcData(POE_calc_data []new_poe_calc){
		int i, j, number_channels;
		float mass1, mass2, temp_mass_ratio;
		float sum;
		POE_calc_data this_poecalcdata;
	...
	public void SetNumTOFs(int new_value)  {num_tofs = new_value;}
	...
	public void SetIsNumDensityCalc(boolean new_bool)  {is_number_density_calc = new_bool;}
	public void SetCalcTOFArray(TOF_data[] new_array)  {calculated_tofs = new_array;}
	// A temporary fix until I am able
	// to change the set-up of my container
	// classes so everything is passed by reference.
	public void RemoveTOF(int TOF_num){
		int i, j, index = 0;

		for(i = 0; i < num_tofs; i++)
		{
			if(TOF_num == included_tof_num_array[i])
			{
				index = i;
				i = num_tofs;
			}
		}
		num_tofs--;  // Decrease the number of TOFs in this calculation by 1
		old_num_tofs = num_tofs;

		TOF_data[] new_calculated_tofs = new TOF_data[num_tofs];
		int[] new_included_tof_num_array = new int[num_tofs];
		float[][][][][] new_TOF_2cos_gamma = new float[num_tofs][][][][];
		float[] new_depolar_major = new float[num_tofs];
		float[] new_depolar_minor = new float[num_tofs];
		float[] new_cos_polar_angle = new float[num_tofs];
		float[] new_sin_polar_angle = new float[num_tofs];
		float[] new_cos_theta_lab = new float[num_tofs];
		float[] new_sin_theta_lab = new float[num_tofs];

		float[] new_tof_lab_angles = new float[num_tofs];
		float[] new_tof_polarization_angles = new float[num_tofs];
		float[] new_depolarization = new float[num_tofs];
		boolean[] new_tof_is_polarized = new boolean[num_tofs];

		float[][][][] new_min_u_squared_array = new float[num_tofs][][][];
		float[][][][] new_max_u_squared_array = new float[num_tofs][][][];
		...
		for(i = 0; i < num_tofs; i++)
		{
			...
			new_calculated_tofs[i] = calculated_tofs[j];
			new_TOF_2cos_gamma[i] = TOF_2cos_gamma[j];
			new_depolar_major[i] = depolar_major[j];
			new_depolar_minor[i] = depolar_minor[j];
			new_cos_polar_angle[i] = cos_polar_angle[j];
			new_sin_polar_angle[i] = sin_polar_angle[j];
			new_cos_theta_lab[i] = cos_theta_lab[j];
			new_sin_theta_lab[i] = sin_theta_lab[j];
			new_tof_lab_angles[i] = tof_lab_angles[j];
			new_tof_polarization_angles[i] = tof_polarization_angles[j];
			new_depolarization[i] = depolarization[j];
			new_tof_is_polarized[i] = tof_is_polarized[j];
			new_included_tof_num_array[i] = included_tof_num_array[j];

			new_min_u_squared_array[i] = min_u_squared_array[j];
			new_max_u_squared_array[i] = max_u_squared_array[j];
		}

		// Need to delete all parts of TOF_2cos_gamma!

		included_tof_num_array = new_included_tof_num_array;

		calculated_tofs = new_calculated_tofs;
		TOF_2cos_gamma = new_TOF_2cos_gamma;
		depolar_major = new_depolar_major;
		depolar_minor = new_depolar_minor;
		cos_polar_angle = new_cos_polar_angle;
		sin_polar_angle = new_sin_polar_angle;
		cos_theta_lab = new_cos_theta_lab;
		sin_theta_lab = new_sin_theta_lab;

		tof_lab_angles = new_tof_lab_angles;
		tof_polarization_angles = new_tof_polarization_angles;
		depolarization = new_depolarization;
		tof_is_polarized = new_tof_is_polarized;
		min_u_squared_array = new_min_u_squared_array;
		max_u_squared_array = new_max_u_squared_array;
	}


	public String GetTitle() {return title;}
	public int GetTotNumPOEs()  {return num_total_poes;}
	public int GetNumContribPOEs()	{return num_poes;}
	public int GetNumTOFs()  {return num_tofs;}
	public float GetIon_m_e() {return ion_m_e;}

	public int GetCalcNumber() {return CalcNumber;}
	public int GetNumBeamAngleSegs() {return num_beam_ang_segs;}
	public int GetNumDetAngleSegs() {return num_det_ang_segs;}
	public int GetNumIonizerSegs() {return num_ionizer_segs;}
	public boolean IsIonizerGaussian() {return is_ionizer_gaussian;}
	public float[] GetIonizerProbDist() {return ionizer_prob_dist;}
	public float[] GetIonizerDistance() {return ionizer_distance;}
	public float GetAlpha() {return alpha;}
	public float GetSpeedRatio() {return speed_ratio;}
	public int GetNumBeamVelSegs()  {return num_beam_vel_segs;}
	public float GetMinLabVel()  {return min_lab_velocity;}
	public float GetMaxLabVel()  {return max_lab_velocity;}
	public int GetNumLabVelSegs()  {return num_lab_vel_segs;}
	public float[] GetTOFLabAngles() {return tof_lab_angles;}
	public float[] GetTOFPolarAngles() {return tof_polarization_angles;}
	public float[] GetDepolarizations() {return depolarization;}
	public boolean[] GetIsPolarized() {return tof_is_polarized;}

	public float GetStartingTime()  {return starting_time;}
	public float GetEndingTime()  {return ending_time;}
	public int GetNumTOFPoints()  {return num_tof_points;}
	...
	public POE_calc_data[] GetPOECalcData()  {return data_for_poes;}
	public POE_calc_data[] GetTempPOECalcData()  {return temp_data_for_poes;}
	...
	/*public bool operator ==(const Calc_data other)
			{
		return (other == this);
			}*/
	...
	public TOF_data[] RunMainFlightTimeCalculation(POE_data[] poes, int num_new_tofs,
		...
		TOF_data[] new_calc_tofs = new TOF_data[num_tofs];
		...
		if(message_dialog.GetStatus() == false)
			...
			for(tof_num = 0; tof_num < num_tofs; tof_num++)
			{
				...
				this_TOF = new TOF_data();
				new_calc_tofs[tof_num] = this_TOF;
				this_TOF.SetIsRealTOF(false);

				this_TOF.SetTitle(tof_title);

				this_TOF.SetDwell(dwell_time_micro);
				this_TOF.SetDwellScale(scale.μs);
				this_TOF.SetOffset(starting_time);
				this_TOF.SetOffsetScale(scale.μs);
				...
				for(poe_num = 0; poe_num < num_total_poes; poe_num++)
				{
					...
					if(current_poe_data.is_included == true)
					{
						...
						this_poe_energy_min = this_poe.GetMinimumEnergy();
						this_poe_energy_max = this_poe.GetMaximumEnergy();
						tof_colors[poe_num] = this_poe.GetPOEColor();

						this_poe_num_points = this_poe.GetTotNumPoints();
						...
						this_poe_energy_amp = this_poe.GetPOEPointer();
				...
				this_TOF.SetTOFPointers(num_total_poes, number_channels, tof_arrays, total_tof_counts);
				//System.out.println(Arrays.deepToString(tof_arrays));
				this_TOF.SetTimePointer(time_array_for_tofs);
				this_TOF.SetTotChannels(num_tof_points);
				this_TOF.SetTOFColors(tof_colors);
				this_TOF.SetNumIncludedPOEs(num_included_poes);
				this_TOF.SetIonFlightTime(0.0f);   // Sets flight const to zero and calculates real flight times
				this_TOF.SetIon_m_e(ion_m_e);
				this_TOF.SetLabAngle(tof_lab_angles[tof_num]);
				this_TOF.SetBeamParams(alpha, speed_ratio, num_beam_vel_segs);
				this_TOF.SetIonizationParams(is_ionizer_gaussian, num_ionizer_segs);
				this_TOF.SetInstrumentParams(is_number_density_calc, flight_length, beam_angular_width,
						detector_angular_width, ionizer_length);
				this_TOF.SetLabVelParams(min_lab_velocity, max_lab_velocity, num_lab_vel_segs);
				this_TOF.SetPolarizationParams(tof_is_polarized[tof_num], tof_polarization_angles[tof_num],
						depolarization[tof_num]);

				this_TOF.SetCalcTitle(title);
				this_TOF.SetAngleSegs(num_beam_ang_segs, num_det_ang_segs);
				this_TOF.SetPOECalcData(data_for_poes);
				this_TOF.SetAssociatedCalc(CalcNumber);
			...
			/*ofstream tempFile("tof.tmp");
		   for(tof_num = 0; tof_num < num_tof_points; tof_num++)
		   	tempFile<<time_array_for_tofs[tof_num] << ",  " << tof_arrays[0][1][tof_num] << ",  " << tof_arrays[0][2][tof_num] << ",  " << tof_arrays[0][0][tof_num] <<endl;
		   tempFile.close(); */
	...
	public boolean GetIsNumDensityCalc()  {return is_number_density_calc;}
	public void CalcTOFDeltaFunctions(int poe_number, POE_data poe, int poe_point){
		/*float time;
		   clock_t start, end;
		    start = clock();     */
		...
		POE_calc_data current_poe_data = data_for_poes[poe_number];
		...
		float poe_amplitude = poe.GetPOEPointer()[poe_point];
		float num_poe_points = poe.GetTotNumPoints();
		...
		for(tof_num = 0; tof_num < num_tofs; tof_num++)
		{
			...
			this_poe_energy_min = poe.GetMinimumEnergy();
			this_poe_energy_max = poe.GetMaximumEnergy();
			//tof_colors[poe_num] = poe.GetPOEColor();

			this_poe_num_points = poe.GetTotNumPoints();
			this_poe_energy_increment = (this_poe_energy_max - this_poe_energy_min) / (this_poe_num_points - 1);
			poe_energy_inc_inverse = (float) (1.0 / this_poe_energy_increment);
			...
			for(chan_num = 0; chan_num < this_num_channels; chan_num++)
			{
				...
				time_array_for_tofs = this_TOF.RealTimePointer();
				for(tof_point_num = 0; tof_point_num < num_tof_points; tof_point_num++)
				{

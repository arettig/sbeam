...
public class ContribPOEsDialog extends JDialog implements ActionListener,
	...
	protected POE_calc_data[] poe_info;
	...
	public ContribPOEsDialog(MainFrame p, sbapp app, Calc_data calc) {
		// TODO Auto-generated constructor stub
		super(p);
		parent = p;

		int i, j, ion_channels;
		POE_calc_data[] calc_poe_calc_data;

		IsOpen = false;

		application = app;
		calculation = calc;
		calc_poe_calc_data = calculation.GetTempPOECalcData();

		calc_title = "";
		poe_title = "";

		calc_num = calculation.GetCalcNumber();
		num_total_poes = calculation.GetTotNumPOEs();
		...
		// poe_calc_data
		poe_info = new POE_calc_data[num_total_poes];
		for (i = 0; i < num_total_poes; i++) {
			poe_info[i] = new POE_calc_data();
		...
		/*
		 * numChan.SetBuddy(HWND(*num_ion_chan_static)); numChan.Attr.Style |=
		 * UDS_SETBUDDYINT; numChan.SetRange(1, 20); numChan.SetPos(1);
		 */

		document = application.GetSessionDocument();
		num_total_poes = document.GetNumPOEs();
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if (e.getSource().equals(ion_chan_listbox)) {
			...
			poe = document.GetPOEData(poe_number);

			poe_title = poe.GetTitle();

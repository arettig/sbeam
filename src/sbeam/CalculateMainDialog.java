...
public class CalculateMainDialog extends JDialog implements ActionListener {
	...
	protected Calc_data calculation;
	...
	public void SetDialogData(Calc_data calc) {
		calculation = calc;
		ion_m_e_edit.setText("" + calculation.GetIon_m_e());
		bm_ang_segs_edit.setText("" + calculation.GetNumBeamAngleSegs());
		det_ang_segs_edit.setText("" + calculation.GetNumDetAngleSegs());
		ionizer_segs_edit.setText("" + calculation.GetNumIonizerSegs());
		alpha_edit.setText("" + calculation.GetAlpha());
		speed_ratio_edit.setText("" + calculation.GetSpeedRatio());
		bm_vel_edit.setText("" + calculation.GetNumBeamVelSegs());
		lab_vel_min_edit.setText("" + calculation.GetMinLabVel());
		lab_vel_max_edit.setText("" + calculation.GetMaxLabVel());
		lab_vel_segs_edit.setText("" + calculation.GetNumLabVelSegs());

		title_edit.setText(calculation.GetTitle());

		num_contrib_poes = calculation.GetNumContribPOEs();

		ionization_is_gaussian = calculation.IsIonizerGaussian();
		calculation_number = calculation.GetCalcNumber();
	}
	...
	protected void ok() {
		TOFPOEDocument document = application.GetSessionDocument();
		...
		calculation.SetTitle(title_edit.getText());
		calculation.SetIon_m_e(Float.parseFloat(ion_m_e_edit.getText()));
		...
		ionizer_length = application.GetSessionDocument().GetIonizerLen();
		...
		calculation.SetNumPOEs(num_contrib_poes);
		document.CalculateTOFs(calculation_number);
	}
	...
	protected void InstrumParams() {
		TOFPOEDocument document;
		document = application.GetSessionDocument();
		Instr_Param_Dialog instr_params = new Instr_Param_Dialog(parent);

		instr_params.SetIonFlightConst("" + document.GetIonFlightConst());

		instr_params.SetIonizerLen("" + document.GetIonizerLen());

		instr_params.SetFlightLen("" + document.GetFlightLen());

		instr_params.SetBeamAng("" + document.GetBeamAng());

		instr_params.SetDetectAng("" + document.GetDetectAng());

		instr_params.SetTypeOfCalculation(document.GetDetectScheme());

		int num_calcs = document.GetNumCalcs();

		if (JOptionPane
				.showConfirmDialog(
						this,
						"This will cause a recalculation of ALL current forward convolutions. Change instrumental parameters?") == JOptionPane.OK_OPTION) {

			instr_params.Execute();

			InstParamsSet = true;
			document.SetIonFlightConst(Float.parseFloat(instr_params
					.GetIonFlightConst()));
			document.SetIonizerLen(Float.parseFloat(instr_params
					.GetIonizerLen()));
			document.SetFlightLen(Float.parseFloat(instr_params.GetFlightLen()));
			document.SetBeamAng(Float.parseFloat(instr_params.GetBeamAng()));
			document.SetDetectAng(Float.parseFloat(instr_params.GetDetectAng()));
			document.SetDetectScheme(instr_params.GetCalcType());
			document.SetIonFlightTimes();
			document.RepaintAllTOFViews();

			application.InstrumentalParamsAreSet(true);
			if (instr_params.GetParamsHaveChanged() == true) {
				document.RecalculateAllCalcs(calculation.GetCalcNumber());
			}
	...
	protected void TOFButton() {
		...
		num_tofs_static.setText("" + calculation.GetNumTOFs());
	...
	public static void main(String[] args) {
		CalculateMainDialog c = new CalculateMainDialog(null, null);
		c.Execute();
	}
